...
public class ProblemOfUsingDirectSQLTests {
    ...
    @DisplayName("직접 SQL 작성해 메뉴를 조회할 때 발생하는 문제 확인")
    @Test
    void testDirectSelectSQL() throws SQLException {
        ...
        while (resultSet.next()) {
            Menu menu = new Menu();
            menu.setMenuCode(resultSet.getInt("MENU_CODE"));
            menu.setMenuName(resultSet.getString("MENU_NAME"));
            menu.setMenuPrice(resultSet.getInt("MENU_PRICE"));
            menu.setCategoryCode(resultSet.getInt("CATEGORY_CODE"));
            menu.setOrderableStatus(resultSet.getString("ORDERABLE_STATUS"));
    ...
    @DisplayName("직접 SQL 작성해 신규 메뉴를 추가할 때 발생하는 문제 확인")
    @Test
...
//            menu.setMenuPrice(rset.getInt("MENU_PRICE"));
    ...
    /* 설명. 4. 동일성 보장 문제 */
    @DisplayName("조회환 두 행의 행을 담은 객체의 동일성 비교 테스트")
    @Test
