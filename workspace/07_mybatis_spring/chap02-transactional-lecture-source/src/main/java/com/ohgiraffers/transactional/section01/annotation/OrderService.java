...
public class OrderService {
     ...
     *    담긴 채로 넘어왔다는 생각을 가지고 개발해 나간다.
     */
    /* 설명. 서비스 메소드 내부에서 모든 DML 관련 작업에 예외없이 잘 동작하면 commit, 예외가 발생하면 rollback이 적용된다. */
    /* 설명. 전파행위 옵션
     ...
     *
     * 설명.
     *  Java는 이러한 정보를 DBMS를 알려주는 역할이고, 실제 정책 수행은 DBMS가 진행한다.
     *  따라서 다른 서버, 다른 언어에서 DBMS에 작업을 하더라도 동시성 문제를 정책에 따른 수준으로 방지할 수 있다.
     */
    ...
    public void registNewOrder(OrderDTO orderInfo) {
        ...
        /* 설명. 2. 주문한 메뉴별로 Menu엔티티에 담아서 조회해오기(부가적인 메뉴의 정보 추출) */
        ...
        /* 설명. 3. 해당 주문건에 대한 총 주문 금액 계산 */
        int totalOrderPrice = calcTotalOrderPrice(orderInfo.getOrderMenus(), menus);
        System.out.println("totalOrderPrice = " + totalOrderPrice);

        /* 설명. 4. 1부터 3까지를 하면 tbl_order 테이블에 삽입(inset)가 가능하다. */
        /* 설명. 4-1. insert를 하기 위해 테이블과 매칭되는 Entity로 옮겨 담는 과정(DTO -> Entity) */
        ...
        System.out.println("after insert order = " + order);
