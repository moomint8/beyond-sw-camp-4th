...
public class ResponseRestController {
    ...
    public List<String> getList() {
        return List.of(new String[]{"사과", "배", "복숭아"});
    }
    ...
    public Map<String, String> getMapping() {
        ...
        return messageList.stream().collect(Collectors.toMap(Message::getMessage, Message::getMessage));
    }

    /* 설명. 이미지 응답하기 */
    /* 설명. produces는 response parameter의 content-type 설정이다. */
    @GetMapping(value = "/image", produces = MediaType.IMAGE_JPEG_VALUE)
    public byte[] getImage() throws IOException {
        return getClass().getResourceAsStream("/static/무민트.png").readAllBytes();
    }
